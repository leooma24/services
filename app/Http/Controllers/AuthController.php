...
class AuthController extends Controller
{
    ...
    public function recover(Request $request) {
        ...
        $details = [
            'subject' => 'MiOrganizador | Recuperar contraseña',
            'token' => $token
        ];

        Mail::to($user->email)->send(new RecoverMailable($details));
    }
    ...
    public function restore_password(Request $request) {
        $validator = Validator::make($request->all(), [
            ...
            'token_recover' => 'required'
        ]);

        if($validator->fails()) {
            return response()->json($validator->errors()->toJSON(), 400);
        }
        $fields = $request->only('email', 'password', 'password_confirmation');
        $fields['token'] = $request->token_recover;
